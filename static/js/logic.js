...
class Frame {
  constructor(node) {
    ...
    this.center = [0, 0];
    this.target = [0.5 * window.innerWidth, 0.5 * window.innerHeight];

    this.padding = [0.5 * this.node.clientWidth, 0.5 * this.node.clientHeight];
  ...
  translate() {
    this.translation.lerp(
      [0, 0, this.mouseover ? 300 : 200 - this.distNorm * 400],
      0.15
    );
    return this;
  }
...
class Gallery {
  constructor() {
    ...
    this.center = [0.5 * window.innerWidth, 0.5 * window.innerHeight];
  ...
  update() {
    this.mouse.lerp(this.mouseover ? this.target : this.center, 0.125);

    this.frames.forEach((frame) => {
      frame.setTarget(this.mouse).setDistance().translate().rotate().update();
    });
